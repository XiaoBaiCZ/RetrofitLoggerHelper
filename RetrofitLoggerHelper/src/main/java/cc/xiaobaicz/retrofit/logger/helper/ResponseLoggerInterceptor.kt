 ...
 */
class ResponseLoggerInterceptor private constructor(val oStream: OutputStream, val isShowHeader: Boolean) : Interceptor {
    companion object {
        /**
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        var rsp = chain.proceed(chain.request())
    ...
    class Builder {
        ...
        fun build(out: Boolean = true): Interceptor = if (out) {
            ResponseLoggerInterceptor(oStream, isShowHeader)
        } else {
            EmptyInterceptor()
        }
    }
